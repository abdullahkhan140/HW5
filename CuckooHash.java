...
public class CuckooHash<K, V> {
	...
	public void put(K key, V value) {
		int pos1 = hash1(key); // two positions in the hash table
		int pos2 = hash2(key);
		Bucket newBucketSpot = new Bucket<>(key, value); // creating new bucket to store pair
		// check if the key pair alr has something in pos1 or pos2, if it does return
		...
		// setup a counter to track attempts
		int attempts = 0;
		// try inserting entry into hashtable
		while (attempts < CAPACITY) {
			// if first position empty, insert newBucketSpot there
			if (table[pos1] == null) {
				table[pos1] = newBucketSpot;
				return;
			}
			// if first position is taken, take out the current entry
			Bucket oldBucketSpot = table[pos1];
			table[pos1] = newBucketSpot;
		...
		// rehash table and try again if attempt exceeds
		rehash();
		// re-insert the kicked out entry after rehashing
		put((K) newBucketSpot.getBucKey(), (V) newBucketSpot.getValue());
	}
